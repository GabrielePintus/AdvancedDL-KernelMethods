...
properties. The gaussian kernel for example defines the space of functions
\begin{align*}
    \mathcal{H}_{\text{gaussian}}   &= \left\{ f : \int | \hat{f}(w) |^2 e^{\frac{\sigma^2 w^2}{2}} dw < \infty \right\} \\
\end{align*}
In other words, the gaussian kernel defines a space of functions that are
smooth and have a finite energy. 


In addition to this we enunciate the following theorems.
...
In the Multiple Kernel Learning framework proposed by Lanckriet, et al. \cite{lanckriet2004multiple},
the coefficients $\eta$ are learned by solving the convex optimization problem.

Now, we define our custom kernel as:
\begin{align*}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        \hline
        KMeans             & \textbf{0.368} & 0.505 & \textbf{0.227} \\
        Agglomerative      & 0.361 & \textbf{0.541} & 0.201 \\
        Gaussian Mixture   & 0.329 & 0.464 & 0.170 \\
        Spectral           & 0.354 & 0.501 & 0.202 \\
        \hline
    \end{tabular}
\end{table}

We decide to adopt the KMeans algorithm.
...
Since it is not clear how to assign some couples (pullover, shirt) and (dress, coat)
...
\end{itemize}
The \enquote{Reduce lr on plateau} learning rate scheduler is used with a factor
...
function is used in order to obtain a vector of probabilities.
...
the validation set. We can see that the performance of the classifiers is
definitely unsatisfactory. This is due to the fact that the labels obtained
from the clustering are not perfect and the classifiers are not able to
generalize well on them.
...
This time the performance of the classifiers is clearly better. This is
due to the fact that the labels are now correct and the classifiers can
better learn the true manifold of the data.
...
when trained on the original dataset. We should also mention that no 
...
We now briefly collect the best results of the experiments in the following table.
\begin{table}[H]
    \centering
    \begin{tabular}{|c|c|c|c|}
        \hline
        ...
        \hline
        SVM (Linear)   & 16.3 & 10.0 & Clustering \\
        SVM (Sigmoid)  & 10.4 & 8.6 & Clustering \\
        CNN            & 16.3 & 10.0 & Clustering \\
        \hline
        SVM (Gaussian) & 83.6 & 81.1 & KPCA \\
        MLP            & 87.1 & 82.5 & KPCA \\
        CNN            & 85.4 & 79.1 & KPCA \\
        \hline
        SVM (Gaussian) & 90.8 & 84.6 & Original \\
        MLP            & 93.9 & 85.0 & Original \\
        CNN            & \textbf{96.9} & \textbf{86.4} & Original \\
        \hline
    \end{tabular}
\end{table}
...
We performed the fully supervised learning experiment twice, once with
the KPCA reduced dataset, with only 10 features, and once with the original dataset, with all the 728 features. We obtained
the best results with the original dataset, where the CNN reached an
accuracy of 86.4\% on the test set.
